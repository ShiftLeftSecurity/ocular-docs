...
```scala
@main def exec(jarFilename: String) = {
  createCpg(jarFilename)
  createSp
  cpg.finding.sortedByScore.l |> outFilename
}
```

This script creates a code property graph and security profile for the input file `jarFilename`, and evaluate the expression `sp.findings.sortedByScore.l` to obtain a list of findings sorted by score. The list is piped to the file `outFilename` via the `|>` operator.
